...
pub const dwarf = struct {
    pub fn crossSectionRelocType(format: DW.Format, cpu_arch: std.Target.Cpu.Arch) u32 {
        return switch (cpu_arch) {
            ...
            .loongarch32, .loongarch64 => @intFromEnum(@as(elf.R_LARCH, switch (format) {
                .@"32" => .@"32",
                .@"64" => .@"64",
            })),
            else => @panic("TODO unhandled cpu arch"),
    ...
    pub fn externalRelocType(
        ...
        return switch (cpu_arch) {
            ...
            .loongarch32, .loongarch64 => @intFromEnum(@as(elf.R_LARCH, switch (source_section) {
                else => switch (address_size) {
                    .@"32" => .@"32",
                    .@"64" => .@"64",
                    else => unreachable,
...
fn formatRelocType(ctx: FormatRelocTypeCtx, writer: *std.Io.Writer) std.Io.Writer.Error!void {
    ...
    switch (ctx.cpu_arch) {
        ...
        .loongarch32, .loongarch64 => try writer.print("R_LARCH_{s}", .{@tagName(@as(elf.R_LARCH, @enumFromInt(r_type)))}),
        else => unreachable,
    }
}
